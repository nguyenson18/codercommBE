...
friendController.sendFriendRequest = catchAsync(async (req, res, next) => {
    ...
    if(!user) throw new AppError(400, 'user not found', "send firend user error")
        ...
        switch (friend.status) {
            case 'pending':
                if(friend.from.equals(currentUserId)){
                    throw new AppError(400, "You have already sent a requests to this user", "Add friend error")
                }else {
...
friendController.getFriendList = catchAsync(async (req, res, next) => {
    ...
    return sendResponse(res, 200, true, {totalPages, count, users},null, 'get list friend success')
});
...
friendController.reactFriendRequest = catchAsync(async (req, res, next) => {
    ...
    return sendResponse(res, 200, true, friend, null, 'React friend request success')
});
...
friendController.cancelFriendRequest = catchAsync(async (req, res, next) => {
    ...
    return sendResponse(res, 200, true,friend, null, 'Friend request has been cancelled')
});
